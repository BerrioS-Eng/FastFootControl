...
export const ProductService = {
    ...
    async edit(id: number, input: Partial<{ name: string; ingredients: CostItem[]; directCosts: CostItem[]; labour: number; profitMargin: number; }>): Promise<Product> {
    ...
    async listForSale(): Promise<Array<{ id: string; product: string; salePrice: number }>> {
        const dtos: ProductSaleTableDTO[] = await ProductsAPI.getSaleTable();
        return dtos.map(d => ({ id: String(d.productId), product: d.productName, salePrice: d.salePrice }));
    },
};